...
public class StatisticsService {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (fileUserId.equals(targetUserId) && startTime.startsWith(todayStr) && type.equals("FOCUS")) {
                    ...
                    // Kiểm tra xem trạng thái có nằm trong các trạng thái cần tính không
                    if (status.equals("COMPLETED") || status.equals("STOPPED_EARLY") || status.equals("CANCELED")) {
    ...
    public Map<LocalDate, Double> getStudyTimeByDay(User user) {
        ...
        for (LocalDate date = monday; !date.isAfter(sunday); date = date.plusDays(1)) {
            studyTimeMap.put(date, 0.0);
        }

        // 3. Duyệt file và cộng dồn thời gian vào đúng ngày trong Map
        try (BufferedReader br = new BufferedReader(new FileReader(SESSION_FILE_PATH))) {
            String line;
            while ((line = br.readLine()) != null) {
                String[] parts = line.split("\\|");
                ...
                // Chỉ lấy ngày từ chuỗi "YYYY-MM-DD"
                LocalDate taskDate = LocalDate.parse(startTime.substring(0, 10));

                // Nếu task thuộc user và nằm trong tuần này
                if (fileUserId.equals(String.valueOf(user.getUserId()))
                        && !taskDate.isBefore(monday) && !taskDate.isAfter(sunday)
                        && (status.equalsIgnoreCase("COMPLETED") || status.equalsIgnoreCase("STOPPED_EARLY") || status.equalsIgnoreCase("CANCELED"))) {

                    double durationHours = Double.parseDouble(parts[5].trim()) / 3600.0;
                    studyTimeMap.put(taskDate, studyTimeMap.get(taskDate) + durationHours);
                }
            }
        ...
        // Làm tròn kết quả
        studyTimeMap.replaceAll((date, hours) -> Math.round(hours * 10.0) / 10.0);
    ...
    public double calculateTaskCompletionRate(User user) {
        ...
        // Lấy ngày Thứ 2 tuần này
        LocalDate startOfWeek = LocalDate.now().with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY));
// Lấy ngày Chủ nhật tuần này
