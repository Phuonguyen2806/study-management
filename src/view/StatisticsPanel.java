    ...
    private JPanel createWeeklyView() {
        ...
        pnlPieChart = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                int y = 30;
                double donePercent = StatisticsPanel.this.doneP;
				double pendPercent = StatisticsPanel.this.pendP;
				double progPercent = StatisticsPanel.this.progP;
				double overduePercent = StatisticsPanel.this.overdueP;
                if (donePercent == 0 && progPercent == 0 && overduePercent == 0) {
                ...
                int angleProg = (int) (progPercent * 3.6);
                int anglePend = (int) (pendPercent * 3.6);
                int angleOverdue = 360 - angleDone - angleProg - anglePend;
                // 2. Vẽ biểu đồ Pie
                g2.setColor(new Color(40, 167, 69));  // Xanh lá (Done)
                g2.fillArc(x, y, size, size, 0, angleDone);

                g2.setColor(new Color(255, 193, 7));  // Vàng (InProgress)
                g2.fillArc(x, y, size, size, angleDone, angleProg);

                g2.setColor(new Color(128, 128, 128));  // Cam (Pending) - MỚI
                g2.fillArc(x, y, size, size, angleDone + angleProg, anglePend);

                g2.setColor(new Color(220, 53, 69));  // Đỏ (Overdue)
                g2.fillArc(x, y, size, size, angleDone + angleProg + anglePend, angleOverdue);

                // 3. Vẽ chữ % lên từng phần bánh
                g2.setColor(Color.WHITE);
                g2.setFont(new Font("Segoe UI", Font.BOLD, 12));
				drawPercentText(g2, x, y, size, 0, angleDone, String.format("%.1f%%", donePercent));
				drawPercentText(g2, x, y, size, angleDone, angleProg, String.format("%.1f%%", progPercent));
				drawPercentText(g2, x, y, size, angleDone + angleProg, anglePend, String.format("%.1f%%", pendPercent));
				drawPercentText(g2, x, y, size, angleDone + angleProg + anglePend, angleOverdue, String.format("%.1f%%", overduePercent));
...
//
			private void drawLegend(Graphics2D g2, int centerX, int y) {
				String[] labels = {"Hoàn thành", "Đang làm", "Chờ xử lý", "Trễ hạn"};
				Color[] colors = {
						new Color(40, 167, 69), // Xanh
				...
				// Khoảng cách giữa các cột và các hàng
				int columnWidth = 100;
				int rowHeight = 20;

				for (int i = 0; i < 4; i++) {
					// Tính toán vị trí:
					// i=0,2 nằm ở cột trái; i=1,3 nằm ở cột phải
					// i=0,1 nằm ở hàng trên; i=2,3 nằm ở hàng dưới
					int col = i % 2;
					int row = i / 2;

					// Vị trí X, Y cụ thể cho từng mục
					int itemX = centerX - 80 + (col * columnWidth);
					int itemY = y + (row * rowHeight);

					g2.setColor(colors[i]);
					g2.fillRoundRect(itemX, itemY, 12, 12, 4, 4);
					g2.setColor(Color.DARK_GRAY);
					g2.drawString(labels[i], itemX + 18, itemY + 10);
				}
			}
        };
    ...
    public void updatePieChartData(double done, double progress, double overdue, double pend) {
        this.doneP = done;
        this.progP = progress;
